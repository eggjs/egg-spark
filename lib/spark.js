...
class Spark extends Readable {
  ...
  async renderBlock(data, index) {
    this.appContext.logger.info(`[egg-spark] RenderBlock ${index} start.`);
    ...
    this.appContext.logger.info(`[egg-spark] RenderBlock ${index} renderTplString start time: ${renderStart}`);
    const renderResult = await this.renderTplString(this.tplPath, blockData);
    this.appContext.logger.info(`[egg-spark] RenderBlock ${index} renderTplString end time: ${Date.now() - renderStart}`);

    const blockHtml = this.getNthBlockHtml(renderResult, this.regList, index);

    this.push(`${blockHtml}`);
    this.appContext.logger.info(`[egg-spark] RenderBlock ${index} end.`);
  }
